...
class Trainer:
    ...
    def train_model_pipeline(X, y, model, param_grid=None, scoring="f1"):
        ...
        if param_grid:
            ...
            grid_search = GridSearchCV(
                pipeline, pipeline_param_grid, cv=cv, scoring=scoring, n_jobs=-1
            )
            grid_search.fit(X, y)
    ...
    def train_from_db(db: Session, param_grid=None, optimize=False):
        ...
        try:
            ...
            trained_pipeline = Trainer.train_model_pipeline(
                X, y, base_model, param_grid=param_grid, scoring="f1"
            )
