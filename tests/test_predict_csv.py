...
def test_predict_csv_success(client, trained_model):
    ...
    response = client.post(
        "/predict-csv", files={"file": ("test.csv", stream, "text/csv")}
    )
...
def test_predict_csv_invalid_format(client):
    ...
    response = client.post(
        "/predict-csv", files={"file": ("test.txt", io.BytesIO(b"hello"), "text/plain")}
    )
...
def test_predict_csv_empty(client, trained_model):
    ...
    stream = io.BytesIO(b"")
    response = client.post(
        "/predict-csv", files={"file": ("empty.csv", stream, "text/csv")}
    )
    assert response.status_code == 400
